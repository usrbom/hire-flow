...
- Tailored resumes should remain export-safe for fixed-template PDF generation. Follow the shared print contract in `Knowledge/my-resumes/base/PDF_EXPORT_SPEC.md` so the Markdown can later be rendered into a submission-ready PDF.
...
- If the user starts a request with `RenderPDF:` followed by a path to a resume Markdown file, treat it as a PDF-generation command and create a submission-ready PDF using `scripts/render_resume_pdf.py`.
